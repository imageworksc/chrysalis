/* Chrysalis Health — homepage concept, option 2
...
(() => {
  ...
  /* ---- steps: the pointer climbs the spine; without one, the scroll does ---- */
  const list = d.getElementById('steps-list');
  const timeline = list ? list.closest('.timeline') : null;
  if (list && timeline) {
    ...
    // to the green on the spine. is-live lights every step up to it.
    const setStep = (step) => {
      items.forEach((li, i) => {
